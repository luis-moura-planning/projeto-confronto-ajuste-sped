        ...
        idx[(num_doc, campo)].append(
            {
                ...
                "cc": str(r["Centro de Custo"])
                if pd.notna(r["Centro de Custo"])
                else "",
                "filial": str(r["Nome da filial"])
                if pd.notna(r["Nome da filial"])
                else "",
...
def compara_gera_diferenca(arquivo_sped: str, planilha_diario: str) -> dict:
    ...
    # Notas só em um lado
    df_so_sped = df[df["_merge"] == "left_only"][
        ["NUM_DOC", "CHV_NFE"] + COLS_SPED
    ].reset_index(drop=True)
    sap_cols_presentes = [
        c
        for c in ["VL_ICMS_SAP", "VL_PIS_SAP", "VL_COFINS_SAP", "VL_ITEM_SAP"]
        if c in df.columns
    ]
    df_so_sap = df[df["_merge"] == "right_only"][
...
def gera_lancamentos_ajuste(
    df_divergencias: pd.DataFrame, df_sap_raw: pd.DataFrame
) -> pd.DataFrame:
    ...
    for _, row in df_divergencias.iterrows():
        ...
        for delta_col, campo in DELTA_PARA_CAMPO.items():
            ...
            sentido = (
                "SAP a maior (estorno)" if delta < 0 else "SAP a menor (complemento)"
            )
