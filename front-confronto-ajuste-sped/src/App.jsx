...
function _tipoLancamento(l) {
  return (l.descricao ?? "").startsWith("Estorno SAP") ? "estornoSap" : "ajuste";
}
...
export default function App() {
  ...
  const [tiposLanc,     setTiposLanc]     = useState({ ajuste: true, estornoSap: false });
  ...
  function mudarFiltro(val) { setFiltro(val); setPaginaComp(1); }

  // ── comparação ─────────────────────────────────────────────────────────────

  const todasLinhas = (resultado?.registros ?? []).map((r) => {
    const tipo  = STATUS_TIPO[r.status] ?? "ok";
    const delta = tipo === "divergencia"
      ? Math.round(Math.abs(_parseV(r.vl_sped) - _parseV(r.vl_sap)) * 100) / 100
      : 0;
  ...
  const linhasFiltradas = todasLinhas.filter((r) => {
    ...
    return (
      ...
      String(r.identificador ?? "").toLowerCase().includes(_busca) ||
      String(r.bloco         ?? "").toLowerCase().includes(_busca) ||
      String(r.imposto       ?? "").toLowerCase().includes(_busca)
    );
  });

  const totalPagsComp = Math.max(1, Math.ceil(linhasFiltradas.length / porPaginaComp));
  const linhasPag     = linhasFiltradas.slice(
    (paginaComp - 1) * porPaginaComp, paginaComp * porPaginaComp
  );

  // ── lançamentos ────────────────────────────────────────────────────────────

  const blocosDisponiveis = resultado
    ? [...new Set((resultado.lancamentos ?? []).map((l) => l.bloco).filter(Boolean))].sort()
    : [];

  const isBlocoAtivo = (bloco) => blocosAtivos === null || blocosAtivos.includes(bloco);

  const _buscaLanc = filtroLanc.trim().toLowerCase();
  const lancamentos = (resultado?.lancamentos ?? [])
    .map((l) => ({ ...l, _imposto: _impostoLanc(l), _tipoLanc: _tipoLancamento(l) }))
    .filter((l) => {
      ...
      return (
        String(l.codigo_da_conta ?? "").toLowerCase().includes(_buscaLanc) ||
        String(l.descricao_conta ?? "").toLowerCase().includes(_buscaLanc) ||
        String(l.descricao       ?? "").toLowerCase().includes(_buscaLanc) ||
        String(l.centro_de_custo ?? "").toLowerCase().includes(_buscaLanc)
      );
    });

  const totalPagsLanc = Math.max(1, Math.ceil(lancamentos.length / porPaginaLanc));
  const lancPag       = lancamentos.slice(
    (paginaLanc - 1) * porPaginaLanc, paginaLanc * porPaginaLanc
  );
  ...
  return (
    <div>
      ...
      <main className="g-container" style={{ marginTop: "var(--g-space-8)", marginBottom: "var(--g-space-8)" }}>
        ...
        >
          ...
          <div className="g-form-grid">
            <DropField
              label="Planilha SAP" required accept=".xlsx"
              file={sapFile} inputRef={sapRef} onChange={setSapFile}
              hint="Clique ou arraste o arquivo .xlsx"
            />
            <DropField
              label="SPED Contribuições" required accept=".txt"
              file={spedFile} inputRef={spedRef} onChange={setSpedFile}
        ...
        {/* ── Resultados ── */}
        {resultado && (
          <section className="g-section" style={{ display: "flex", flexDirection: "column", gap: "var(--g-space-5)" }}>

            {/* Cards de resumo */}
            <div className="g-grid g-grid--auto-160">
              <ResumoCard label="Divergências" valor={todasLinhas.filter((r) => r._tipo === "divergencia").length} cor="warn" />
              <ResumoCard label="Só no SPED"   valor={todasLinhas.filter((r) => r._tipo === "so_sped").length}    cor="danger" />
              <ResumoCard label="Só no SAP"    valor={todasLinhas.filter((r) => r._tipo === "so_sap").length}     cor="" />
              <ResumoCard label="Lançamentos"  valor={lancamentos.length}                                          cor="" />
            </div>

            {/* Abas */}
            <div className="g-tabs">
              <button className={`g-tabs__item${abaAtiva === "comparacao"  ? " g-tabs__item--active" : ""}`} onClick={() => setAbaAtiva("comparacao")}>
                Comparação
              </button>
              <button className={`g-tabs__item${abaAtiva === "lancamentos" ? " g-tabs__item--active" : ""}`} onClick={() => setAbaAtiva("lancamentos")}>
                Lançamentos
                {lancamentos.length > 0 && (
                  <span className="g-badge g-badge--neutral" style={{ marginLeft: 6 }}>{lancamentos.length}</span>
                )}
              </button>
            </div>
              ...
              <>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "var(--g-space-2)" }}>
                  <div style={{ display: "flex", flexDirection: "column", gap: "var(--g-space-1)" }}>
                    <div className="g-cluster" style={{ gap: "var(--g-space-1)" }}>
                      {[["todos","Todos"],["divergencia","Divergências"],["ok","OK"]].map(([val, label]) => (
                        <button key={val} className={`g-pill${filtro === val ? " g-pill--active" : ""}`} onClick={() => mudarFiltro(val)}>{label}</button>
                      ))}
                    </div>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: "var(--g-space-2)" }}>
                    <input
                      className="g-input" style={{ width: 260 }}
                      placeholder="Buscar por nota, chave, bloco ou imposto…"
                      value={filtroTexto}
                      onChange={(e) => { setFiltroTexto(e.target.value); setPaginaComp(1); }}
                    />
                    {filtroTexto && (
                      <button className="g-btn g-btn--sm" onClick={() => { setFiltroTexto(""); setPaginaComp(1); }} title="Limpar busca">×</button>
                    )}
                    <span className="g-helper">{linhasFiltradas.length} registro{linhasFiltradas.length !== 1 ? "s" : ""}</span>
                  </div>
                </div>

                <BarraPaginacao
                  pagina={paginaComp} total={totalPagsComp} porPagina={porPaginaComp}
                  totalItens={linhasFiltradas.length} onPagina={setPaginaComp}
                ...
                <div className="g-table-wrap">
                  <table className="g-table">
                    ...
                    <tbody>
                      {linhasPag.length === 0 && (
                        <tr><td colSpan={8} className="g-empty">Nenhum resultado.</td></tr>
                      )}
                      {linhasPag.map((r, i) => (
                        <tr key={i} className={r._tipo === "divergencia" ? "app-row-diff" : ""}>
                          <td>{r.num_doc || "—"}</td>
                          <td>
                            {r.identificador
                              ? <code className="g-mono" style={{ fontSize: 11, whiteSpace: "nowrap" }}>{r.identificador}</code>
                              : "—"}
                          </td>
                          <td>
                          ...
                          <td>
                            <span className="g-badge g-badge--neutral">{r.imposto}</span>
                          </td>
                          <td>
              ...
              <>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "var(--g-space-3)" }}>
                    ...
                    <div className="g-cluster" style={{ gap: "var(--g-space-4)", paddingBottom: "var(--g-space-2)", borderBottom: "1px solid var(--g-border)" }}>
                      ...
                      {IMPOSTOS_LANC.map(({ campo, label }) => (
                        <label key={campo} className="g-check">
                          <input type="checkbox" checked={impostosAtivos[campo]} onChange={() => toggleImposto(campo)} />
                          {label}
                        </label>
                      ))}
                    </div>
                    <div className="g-cluster" style={{ gap: "var(--g-space-4)", paddingTop: "var(--g-space-2)" }}>
                      <span className="g-helper" style={{ fontSize: 11 }}>Tipo:</span>
                      {TIPOS_LANC.map((t) => (
                        <label key={t.key} className="g-check" style={t.style ?? undefined}>
                          <input type="checkbox" checked={tiposLanc[t.key]} onChange={() => toggleTipoLanc(t.key)} />
                          {t.label}
                        </label>
                      ))}
                    </div>
                    {blocosDisponiveis.length > 0 && (
                      <div className="g-cluster" style={{ gap: "var(--g-space-3)", paddingTop: "var(--g-space-2)", borderTop: "1px solid var(--g-border)", flexWrap: "wrap" }}>
                        <span className="g-helper" style={{ fontSize: 11 }}>Bloco:</span>
                        {blocosDisponiveis.map((bloco) => (
                          <label key={bloco} className="g-check">
                            <input type="checkbox" checked={isBlocoAtivo(bloco)} onChange={() => toggleBloco(bloco)} />
                            {bloco}
                          </label>
                        ))}
                        <button
                          ...
                          onClick={() => toggleTodosBlockos(blocosAtivos !== null)}
                        >
                          {blocosAtivos === null ? "Desmarcar todos" : "Selecionar todos"}
                        </button>
                      </div>
                    )}
                  </div>
                  <div className="g-cluster" style={{ gap: "var(--g-space-2)" }}>
                    <input
                      className="g-input" style={{ width: 280 }}
                      placeholder="Buscar por conta, descrição ou C.Custo…"
                      value={filtroLanc}
                      onChange={(e) => { setFiltroLanc(e.target.value); setPaginaLanc(1); }}
                    />
                    {filtroLanc && (
                      <button className="g-btn g-btn--sm" onClick={() => { setFiltroLanc(""); setPaginaLanc(1); }} title="Limpar busca">×</button>
                    )}
                    <span className="g-helper">{lancamentos.length} lançamento{lancamentos.length !== 1 ? "s" : ""}</span>
                    <button type="button" className="g-btn g-btn--orange" disabled={!lancamentos.length} onClick={() => exportarXLSX(lancamentos)}>
                      ↑ Exportar XLSX
                    </button>
                  </div>
                </div>
                ...
                {lancamentos.length === 0 ? (
                  ...
                  <>
                    <BarraPaginacao
                      pagina={paginaLanc} total={totalPagsLanc} porPagina={porPaginaLanc}
                      totalItens={lancamentos.length} onPagina={setPaginaLanc}
                    ...
                    <div className="g-table-wrap">
                      <table className="g-table">
                        ...
                        <tbody>
                          {lancPag.map((l, i) => (
                            <tr key={i} className={l._tipoLanc === "estornoSap" ? "app-row-estorno-sap" : ""}>
                              <td><code className="g-mono">{l.codigo_da_conta}</code></td>
                              <td style={{ maxWidth: 200 }}><Trunc>{l.descricao_conta}</Trunc></td>
                              <td className={l.debito != null ? "app-td-debito" : ""}>
                                {fmt(l.debito)}
                              </td>
                              <td className={l.credito != null ? "app-td-credito" : ""}>
                                {fmt(l.credito)}
                              </td>
                              <td style={{ maxWidth: 240 }}><Trunc maxW={230}>{l.descricao}</Trunc></td>
                              <td>{l.centro_de_custo}</td>
                              <td>{l.filial}</td>
                              <td>
                                {l._imposto && (
                                  <span className="g-badge g-badge--neutral">{l._imposto}</span>
                                )}
                              </td>
...
function BarraPaginacao({ pagina, total, porPagina, totalItens, onPagina, onPorPagina }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "var(--g-space-2)" }}>
      <div className="g-cluster" style={{ gap: "var(--g-space-2)" }}>
        <span className="g-helper">{inicio}–{fim} de {totalItens}</span>
        <select className="g-select" style={{ width: "auto", padding: "4px 8px" }} value={porPagina} onChange={(e) => onPorPagina(Number(e.target.value))}>
          {OPCOES_POR_PAGINA.map((n) => <option key={n} value={n}>{n} por página</option>)}
        </select>
      </div>
      {total > 1 && (
        <div className="g-cluster" style={{ gap: "var(--g-space-1)" }}>
          ...
          {pages.map((p, i) =>
            p === "..." ? (
              ...
              <button key={p} className={`g-btn g-btn--sm${p === pagina ? " g-btn--primary" : ""}`} onClick={() => onPagina(p)}>{p}</button>
            )
          )}
  ...
  return (
    <div className="g-field">
      <label className="g-field__label">
        {label} {required && <span style={{ color: "var(--g-danger)" }}>*</span>}
      </label>
      <div
...
function Trunc({ children, maxW = 220 }) {
  ...
  return (
    <span title={text} style={{ display: "block", maxWidth: maxW, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
      {children}
    </span>
  );
}
